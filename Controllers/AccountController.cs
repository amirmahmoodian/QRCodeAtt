...
using QRCodeAttendance.Controllers;
...
namespace IdentitySample.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        // GET: Movies
        public async Task<IActionResult> log(DateTime startdate, DateTime enddate, string SearchString)
        {
            var log = from m in _context.log
                      select m;
            log = log.Where(s => s.UserName == System.Security.Principal.WindowsIdentity.GetCurrent().Name);
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Register([Bind("Id,UserName,UserCode")] QRCodeModel code, RegisterViewModel model, string returnUrl = null)
        {
            ViewData["ReturnUrl"] = returnUrl;
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        //return View();
                        returnUrl = "https://localhost:5001/Home/Indexuser/" + code.Id ;
